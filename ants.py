...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        current_place = self.place
        current_place_bees  = list(current_place.bees)
        self.armor -= amount 
        if self.armor <= 0:
            survived_bees = [b.reduce_armor(self.damage)for b in current_place_bees]
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        current_place = self.place
        current_place_bees  = list(current_place.bees)
        attack_bees = [b.reduce_armor(self.damage) for b in current_place_bees if current_place_bees]
        protect_ants = BodyguardAnt.action(self,colony)
        # END Problem 10
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_true_queen == False:
            ...
            while current_place:
                if current_place.ant and current_place.ant not in QueenAnt.doubled_list:
                    current_place.ant.damage *= 2
                    QueenAnt.doubled_list.append(current_place.ant)
                if current_place.ant and current_place.ant.is_container:
                    if current_place.ant.contained_ant and current_place.ant.contained_ant not in QueenAnt.doubled_list:
                        current_place.ant.contained_ant.damage *= 2
                        QueenAnt.doubled_list.append(current_place.ant.contained_ant)
