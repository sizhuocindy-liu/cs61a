...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    total = 0
    if num_rolls == 0:
        total = free_bacon(opponent_score)
    else: 
        while i < num_rolls:
            a = dice()
            if a == 1:
...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    tens_player = player_score // 10 % 10
    ones_player = player_score % 10
    tens_opponent = opponent_score // 10 % 10 
    ones_opponent = opponent_score % 10 
    diff_player = abs(tens_player - ones_player)
    diff_opponent = abs(tens_opponent - ones_opponent)
    return diff_player == diff_opponent 
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    n, current_max, max_num_rolls = 1, 0, 0
    outcome = make_averaged(roll_dice, num_samples)
    while n <= 10:
        new_turn = outcome(n, dice)
        if new_turn > current_max:
            current_max = new_turn
            max_num_rolls = n 
        n = n + 1
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    new_tens = total_score // 10 
    new_ones = total_score % 10 
    diff_opponent = abs(tens - ones)
    diff_score = abs(new_tens - new_ones)
    if total <= 0:
        total = 1
    if total_score < opponent_score and diff_opponent == diff_score: 
        return 0 
    elif diff_opponent != diff_score and total >= margin:
        return 0 
    else:
        return num_rolls
    # END PROBLEM 11
